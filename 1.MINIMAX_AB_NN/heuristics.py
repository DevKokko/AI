...
import math
from NN_board_evaluation import get_NN_board_evaluation
from tensorflow.keras.backend import get_value as tf_get_value
...
def NN_heuristic(board, placement_phase, player_turn, AI_marker, made_mills_indexes, men_remaining, participating_mills):
    evaluation = get_NN_board_evaluation(board)
    evaluation = tf_get_value(evaluation)[0][0]
    if player_turn == AI_marker: # human (min player) has just played
        return evaluation
    else: # AI (max player) has just played
        return -evaluation