...
def get_environment(env_name):
    try:
        ...
        elif env_name in ('ninemensmorris'):
            from ninemensmorris.envs.ninemensmorris import NineMensMorrisEnv
            return NineMensMorrisEnv
        elif env_name in ('nmm'):
            from nmm.envs.nmm import NMMEnv
            return NMMEnv
        else:
...
def get_network_arch(env_name):
    if env_name in ('tictactoe'):
        ...
        from models.ninemensmorris.models import CustomPolicy
        return CustomPolicy
    elif env_name in ('nmm'):
        from models.nmm.models import CustomPolicy
        return CustomPolicy
    else:
